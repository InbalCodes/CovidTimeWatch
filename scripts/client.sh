...
sleep 0.1
curl -X "POST" -H "Content-Type: application/json" -d '{"name":"inbal"}' localhost:3000/employees/leave
curl -X "POST" -H "Content-Type: application/json" -d '{"name":"barak"}' localhost:3000/employees/leave
sleep 0.1
...
curl "localhost:3000/employees/history?name=david"
